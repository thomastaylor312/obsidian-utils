...
fn arithmetic_on_numbers() {
    ...
    assert_eq!(product, Value::Number(15.0));

    let quotient = a.div(&b).expect("div numbers");
    assert_eq!(quotient, Value::Number(5.0 / 3.0));

    let quotient_int = Value::Number(6.0)
        .div(&Value::Number(3.0))
        .expect("div integers");
    assert_eq!(quotient_int, Value::Number(2.0));
}
...
#[test]
fn comparisons_work() {
    let cmp = Value::Number(2.0)
        .compare(&Value::Number(10.0))
...
#[test]
fn list_contains() {
    let list = Value::List(ListValue::new(vec![Value::Number(1.0), Value::Number(2.0)]));
    assert!(
        list.contains(&Value::Number(2.0))
...
fn display_formatting() {
    ...
    let integer_display = Value::Number(42.0).to_string();
    assert_eq!(integer_display, "42");

    let float_display = Value::Number(2.1200).to_string();
    assert_eq!(float_display, "2.12");
}

#[test]
fn mixed_numeric_addition() {
    let result = Value::Number(2.5)
...
fn division_and_remainder_by_zero_error() {
    let err = Value::Number(4.0)
        .div(&Value::Number(0.0))
        .expect_err("division by zero fails");
    assert_invalid_operation(err, "div", "number", "number");

    let err = Value::Number(10.0)
        .rem(&Value::Number(0.0))
...
fn contains_errors_on_non_collection_values() {
    let err = Value::Number(2.0)
        .contains(&Value::Number(2.0))
...
fn len_errors_on_unsupported_types() {
    let err = Value::Number(2.0).len().expect_err("len on number fails");
